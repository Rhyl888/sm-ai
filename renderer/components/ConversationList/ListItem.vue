...
const isTitleEditTable = computed(() => ctx?.editId.value === props.id);
...
<template>
  ...
  <div class="w-full flex items-center" v-if="isBatchOperate">
    ...
    <div class="flex-auto">
      <item-title :title="title" :is-edit-table="isTitleEditTable" @update-title="updateTitle" />
    </div>
  </div>
  <item-title v-else :title="title" :is-edit-table="isTitleEditTable" @update-title="updateTitle" />
</template>
