  ...
  [
    ...
    async () => {
      checkedIds.value.forEach((id) => {
        if (
          conversationsStore.allConversations.find((item) => item.id === id)
            ?.pinned
        ) {
...
function updateTitle(id: number, title: string) {
  const target = conversationsStore.conversations.find(
    (item) => item.id === id
  );
  if (!target) return;
    ...
    <ul class="flex-auto overflow-auto">
      <template v-for="item in conversations" :key="item.id">
        <li v-if="item.type !== 'divider'"
          ...
          @contextmenu.prevent.stop="handleItemContextMenu(item)" @click="handleItemClick(item)">
